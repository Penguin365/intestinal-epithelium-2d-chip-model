/*
 * 2D chip-inspired intestinal epithelial model demo with neighbour- and height-regulated proliferation.
 *
 ...
 * chip-inspired epithelial model, including the modified proliferation rule,
 * neighbour-number-dependent contact inhibition, height-dependentproliferation
 * restriction, and simulation setup.
 */

#ifndef TEST_CHIP_FRAME_2D_NH_DEMO_HPP_
#define TEST_CHIP_FRAME_2D_NH_DEMO_HPP_
...
class TestChipFrame_2D_NH_demo : public AbstractCellBasedTestSuite
{
private:
    ...
    void RunSimulation(double h_thresh, double tp, double sr, const std::string& outputDir)
    {
        // Reseed each run.
        ...
        {
            ConfigurableContactInhibitionCellCycleModel* p_model =
                new ConfigurableContactInhibitionCellCycleModel();
            p_model->SetUseNeighbourInhibition(true);
            p_model->SetUseHeightInhibition(true);
            p_model->SetNeighbourThreshold(4);
            p_model->SetHeightThreshold(h_thresh);
            p_model->SetAllowHardCellDivision(false);
        ...
        // Initialise CellData fields required.
        for (typename AbstractCellPopulation<2>::Iterator cell_iter = cell_population.Begin();
             cell_iter != cell_population.End(); ++cell_iter)
        {
            cell_iter->GetCellData()->SetItem("neighbour_count", 0.0);
            cell_iter->GetCellData()->SetItem("height", 0.0);
        }
        ...
        MAKE_PTR(HeightTrackingModifier<2>, p_h_modifier);
        simulator.AddSimulationModifier(p_h_modifier);
...
public:
    void Test2DChipModelNHDemo()
    {
        EXIT_IF_PARALLEL;

        // Single compact demo parameter set.
        const double h_value = 3.0;
        const double tp = 0.3;
        const double sr = 1.0;
        const int replicate = 1;

        std::ostringstream oss;
        oss << "ChipFrame2D_demo/NH_demo"
            << "_h" << std::fixed << std::setprecision(1) << h_value
            << "_tp" << std::setprecision(1) << tp
        ...
        try {
            TS_TRACE(">>> START: " + dir);
            RunSimulation(h_value, tp, sr, dir);
            TS_TRACE(">>> DONE:  " + dir);
        } catch (Exception& e) {
...
#endif // TEST_CHIP_FRAME_2D_NH_DEMO_HPP_
